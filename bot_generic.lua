...
function Retreat()
	if (team == 3) then
		if (pID == 7 or pID == 8) then
			MoveDirectly(npcBot, GetLocation(npcBot) + Vector(0, 100, 0))
		elseif (pID == 9 or pID == 10) then
			MoveDirectly(npcBot, GetLocation(npcBot) + Vector(0, 100, 0))
		else
			MoveDirectly(npcBot, GetLocation(npcBot) + Vector(100, 100, 0))
		end
	else
...
function Think()
	...
	local WeakestAlly,AllyHealth = module.GetWeakestUnit(Alliedcreeps)
...
----If no other actions, then move to lane (first 20 seconds)----
	...
	if (GameTime <= 1200) then
		...
		if (WRDByTower(npcBot, 0.5) or WRDByCreep(npcBot, 0.5) or WRDByHero(npcBot, 0.5)) then
			Retreat()
		end
		...
		if (WeakestCreep ~= nil and percentHealth > 0.2 and CreepHealth <= npcBot:GetAttackDamage() * 1.2) then
			if (GetUnitToUnitDistance(npcBot,WeakestCreep) <= ARange) then
				...
				--if (GetUnitToUnitDistance(npcBot,WeakestCreep) <= ARange) then
				--	AttackUnit(npcBot, WeakestCreep, true)
				--end
			end
		----Deny creep----
		elseif (WeakestAlly ~= nil and percentHealth > 0.2 and AllyHealth <= npcBot:GetAttackDamage()) then
			if (GetUnitToUnitDistance(npcBot,WeakestAlly) <= ARange) then
				AttackUnit(npcBot, WeakestAlly, false)
			end
		end
